...
export function spin(_s, d) {
  ...
  return t(`scale(${1 + (s * 0.003)})
    translateX(${-2 * s * d}px)
    translateY(${-dy}px) rotate(${az}deg) translateY(${dy}px)`);
}

export function cardMove(s, hh, r) {
  if (s === 0 || isMobile()) {
    return {};
  }
  if (s < 0) {
...
export function cardMoveBack(s, hh, r) {
  if (s === 0 || isMobile()) {
    return {};
  }
  var notimes = Math.min(1, s / hh);
...
export function cardMoveEdge(s, hh, r) {
  if (s === 0 || isMobile()) {
    return {};
  }
  // return {};
